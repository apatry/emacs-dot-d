...
run its ERT suite in the background and report results in a
=*journal-reflect-tests*= compilation buffer. This only hooks that one
file (via =emacs-lisp-mode-hook= checking the filename), not every
Elisp file, and doesn't require =~/journal= to exist.

#+begin_src emacs-lisp
  (defun journal-reflect--test-command ()
    (let ((dir (expand-file-name "~/.emacs.d/elisp/")))
      (format "emacs -Q --batch -L %s -l journal-reflect -l journal-reflect-test -f ert-run-tests-batch-and-exit"
              (shell-quote-argument dir))))

  (defun journal-reflect-run-tests ()
    "Run the journal-reflect ERT suite in a compilation buffer."
    (interactive)
    (let ((default-directory (expand-file-name "~/.emacs.d/elisp/")))
      (compilation-start (journal-reflect--test-command)
                          nil
                          (lambda (_mode) "*journal-reflect-tests*"))))

  (defun journal-reflect--hook-tests-on-save ()
    (when (equal (file-name-nondirectory (or buffer-file-name "")) "journal-reflect.el")
      (add-hook 'after-save-hook #'journal-reflect-run-tests nil t)))

  (add-hook 'emacs-lisp-mode-hook #'journal-reflect--hook-tests-on-save)
#+end_src
